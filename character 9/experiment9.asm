...
datasg segment
data:	db	'welcome to masm!'  ; 16bytes
        db	0BH
...
start:
    ...
    mov bx,offset data       ; 取得标号对ds:0的偏移，16 ds:[bx]之后为存放行数的,因为存在头部之类的原因，需要偏移100H，跳过头部
    add bx,010H              ; 再偏移10H到达 0BH在ds存放的偏移地址
    mov cx,3                 ; 复制3行

    mov al,0A0H              ; 1行80 字符，160bytes
    mul byte ptr ds:[bx]     ; 0BH * 0A0H 定位到屏幕12行
    add ax,60                ; 定位到行中间,ax 存放着偏移地址,10进制，懒得换算
    mov di,ax
    add di,020h              ; 指向打印后字符串末尾，方便循环用,字符串长16bytes，加上颜色信息32bytes = 020H
    inc bx                   ; bx 指向颜色的区域,此时bx 指向颜色部分

next:
    mov si,offset data       ; 字符串起始偏移
